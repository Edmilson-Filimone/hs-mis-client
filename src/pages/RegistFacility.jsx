...
const postData = async (data)=>{
  try {
    await axios.post('http://localhost:8080/facility', data)
    return true
  } catch (error) {
    console.log(error)
  ...
  return (
    <>
      <div className="w-full mt-10 py-8 px-4">
        <h3 className="text-center mx-auto text-2xl text-title-color font-semibold">Add new Health Facility</h3>
        <form className="flex flex-col space-y-5 items-center max-w-[700px] w-full mx-auto py-5 text-text-color" onSubmit={onsubmit}>
          ...
          <input className="w-full sidebar-item-last" type="submit" value='Add Facility'/>
        </form>
