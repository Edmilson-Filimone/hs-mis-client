...
function EditSample() {
  ...
  const onsubmit = (e)=>{
    ...
    formData.resistence = 'No' //TEMPORARY QUANDO REFACTORAR API MODIFICAR ESTA SECCAO E AJSUTAR O FILTO DE USEPROSSDATA 
...
const putData = async (data)=>{
  try {
    console.log(data)
    await axios.put(`http://localhost:8080/sample/${params.id}`, data)
    return true
  } catch (error) {
    console.log(error)
  ...
  const uploadSpreadSheet = async (e)=>{
    ...
    try {
      ...
      fileObject.forEach((item)=> {
        ...
        axios.put(`http://localhost:8080/sample/${item.id}`, item)
      })
...
useEffect(()=>{
  async function getData(){
    const data_a = axios.get(`http://localhost:8080/sample/${params.id}`)
    const data_b = axios.get(`http://localhost:8080/facility?max=1000`)
    axios.all([data_a, data_b]).then(
      axios.spread((...allData) =>{
        const dataA = allData[0].data
        const dataB = allData[1].data
        setFormData(dataA)
        console.log(dataA)
        setFormData((prevStat)=>({...prevStat, date:`${dataA.date.split('T')[0]}`})) //formatting date to fit in the date input pattern
        setSelectFacility(dataB)
        setIsBusy(false)
      })
    )
  }
  ...
  return (
    <>
      <div className="w-full mt-10 py-8 px-4">
        <h3 className="text-center mx-auto text-2xl text-title-color font-semibold">Edit Sample</h3>
        <form className="flex flex-col space-y-5 items-center max-w-[700px] w-full mx-auto py-5 text-text-color" onSubmit={onsubmit}>
          ...
          <div className='input-container'>
              ...
              <select className="input-style"  name='facility' id='facility' value={facility} onChange={onchange} onClick={onchange}>
                <option key={1} value={facility.id}>{facility.name}</option>
          ...
          <input className="w-full sidebar-item-last" type="submit" value='Save Changes'/>
        </form>
